...
async function login(){
  ...
  button.textContent="Googleを開いています…";
  ...
  try{
    ...
    if(error.code==="auth/popup-blocked"){
      alert("Googleログイン画面を開けませんでした。Safariのポップアップブロックを一時的にOFFにして、もう一度押してね。");
    }else if(error.code==="auth/unauthorized-domain"){
      alert("このサイトのドメインがFirebaseで許可されていません。Firebase Authenticationの承認済みドメインに millydisney.github.io を追加してね。");
    }else if(error.code==="auth/popup-closed-by-user" || error.code==="auth/cancelled-popup-request"){
      // ユーザーが閉じた場合は何もしない
    }else{
      alert(`Googleログインできませんでした。
