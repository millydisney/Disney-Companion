...
const categoryInfo = {
  ...
  show:["ショー・パレード","show"], greeting:["グリーティング","greeting"],
  experience:["予約・体験","experience"]
};

const DEVICE_KEY = "dcDeviceId";
const NAME_KEY = "dcDisplayName";
const ICON_KEY = "dcProfileIcon";
...
function getIcon(){ return localStorage.getItem(ICON_KEY) || "🎀"; }
...
function rememberGroup(code,plan,role){
  ...
  groups[code]={code,name:meta.name||"みんなのディズニープラン",date:meta.date||"",park:meta.park||"",role:role||"member",lastOpened:Date.now()};
...
function refreshNameUI(){
  const name=getName(), icon=getIcon();
  $("#homeNameText").textContent=name;
  $("#profileNameInput").value=name==="ゲスト"?"":name;
  $("#currentNameText").textContent=name;
  $("#homeAvatarPreview").textContent=icon;
  $("#profileAvatarPreview").textContent=icon;
...
function saveProfile(){
  ...
  const selected=$("#iconPicker button.selected");
  const icon=selected?.dataset.icon||getIcon();
  localStorage.setItem(NAME_KEY,value);
  localStorage.setItem(ICON_KEY,icon);
  refreshNameUI();
  $("#profileDialog").close();
  if(currentCode) updateMyMemberProfile(value,icon);
  showToast("プロフィールを保存しました");
}
async function updateMyMemberProfile(name,icon){
  try{
    await update(ref(db,`plans/${currentCode}/members/${deviceId()}`),{
...
async function renderGroupList(){
  ...
  const upcoming=[], today=[], history=[];

  entries.forEach(group=>{
    if(!group.date){ upcoming.push(group); return; }
    if(group.date===todayStr) today.push(group);
    else if(group.date<todayStr) history.push(group);
    else upcoming.push(group);
  });

  $("#upcomingCount").textContent=`${upcoming.length}件`;
  $("#todayCount").textContent=`${today.length}件`;
  $("#historyCount").textContent=`${history.length}件`;

  renderGroupBucket("#upcomingList",upcoming,"これからのプランはまだありません。");
  renderGroupBucket("#todayList",today,"今日のプランはありません。");
  renderGroupBucket("#historyList",history,"履歴はまだありません。");
}

function renderGroupBucket(selector,entries,emptyText){
  const list=$(selector); list.innerHTML="";
  if(!entries.length){
    list.innerHTML=`<div class="empty-groups">${emptyText}</div>`;
    return;
  }
...
async function createPlan(){
  ...
  try{
    ...
    const data={
      meta:{name:"みんなのディズニープラン",date:today,park:"東京ディズニーランド",createdBy:name,ownerId:id,createdAt:serverTimestamp(),updatedAt:serverTimestamp()},
      members:{[id]:{name,icon:getIcon(),role:"owner",joinedAt:serverTimestamp(),updatedAt:serverTimestamp()}},
      items:{}
    };
...
async function findPlanForJoin(codeValue){
  ...
  try{
    ...
    $("#invitePlanName").textContent=meta.name||"みんなのディズニープラン";
...
async function joinPlan(codeValue,{skipConfirm=false}={}){
  ...
  try{
    ...
    await update(ref(db,`plans/${code}/members/${id}`),{
      name:getName(),icon:getIcon(),role:isOwner?"owner":"member",joinedAt:plan.members?.[id]?.joinedAt||serverTimestamp(),updatedAt:serverTimestamp()
    });
...
function render(){
  ...
  items.forEach(item=>{
    ...
    el.innerHTML=`<div class="item-time">${item.start?esc(item.start):"時間未定"}${item.end?`<br><small>〜${esc(item.end)}</small>`:""}</div>
      <div><span class="badge ${cls}">${label}</span><div class="item-title">${esc(item.title)}</div>
      ${item.note?`<div class="item-note">${esc(item.note)}</div>`:""}<div class="item-author">追加：${esc(item.authorIcon||"👤")} ${esc(item.author||"ゲスト")}</div></div>
  ...
  const nextTimed=items.find(item=>!item.done && item.start);
  const nextUntimed=items.find(item=>!item.done && !item.start);
  const next=nextTimed||nextUntimed;
  $("#nextTime").textContent=nextTimed?.start||(nextUntimed?"時間未定":"--:--");
  $("#nextTitle").textContent=next?.title||"予定を追加しよう";
  $("#nextNote").textContent=next?(next.note||`追加：${next.authorIcon||"👤"} ${next.author||"ゲスト"}`):"みんなの追加・変更がここに反映されます";
}
...
function queueMetaUpdate(){
  ...
  metaTimer=setTimeout(async()=>{
    ...
    try{await update(ref(db,`plans/${currentCode}/meta`),{
      name:$("#planNameInput").value.trim()||"みんなのディズニープラン",date:$("#planDateInput").value,
...
function openNew(){
  editingId=null;$("#dialogTitle").textContent="予定を追加";$("#categoryInput").value="dpa";
...
async function saveItem(){
  ...
  if(!title){alert("予定名を入力してね");return;}
  ...
  const data={category:$("#categoryInput").value,title,start,end:$("#endTimeInput").value,note:$("#noteInput").value.trim(),
    author:editingId?(old.author||getName()):getName(),authorIcon:editingId?(old.authorIcon||getIcon()):getIcon(),done:editingId?Boolean(old.done):false,updatedBy:getName(),updatedAt:serverTimestamp()};
...
async function openMembers(){
  ...
  Object.entries(members).sort((a,b)=>(a[0]===ownerId?-1:b[0]===ownerId?1:0)).forEach(([id,m])=>{
    ...
    row.innerHTML=`<span class="member-avatar">${id===ownerId?"👑":esc(m.icon||"👤")}</span>
...
document.querySelectorAll("#iconPicker button").forEach(btn=>{
  btn.addEventListener("click",()=>{
    ...
    $("#profileAvatarPreview").textContent=btn.dataset.icon;
  });
});
